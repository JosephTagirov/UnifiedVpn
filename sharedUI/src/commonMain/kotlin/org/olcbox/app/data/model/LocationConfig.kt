...
data class VpnProfileConfig(
    val type: String = "olcrtc",
    val name: String? = null,
    val uri: String? = null,
    @SerialName("raw_config")
    val rawConfig: String? = null,
    @SerialName("local_socks_host")
    val localSocksHost: String? = null,
    @SerialName("local_socks_port")
    val localSocksPort: Int? = null
) {
    val normalizedType: String
        get() = normalizeType(type)

    fun normalized(): VpnProfileConfig {
        return copy(
            type = normalizedType,
            name = name.cleanMetadataValue(),
            uri = uri.cleanMetadataValue(),
            rawConfig = rawConfig.cleanMetadataValue(),
            localSocksHost = localSocksHost.cleanMetadataValue(),
            localSocksPort = localSocksPort?.takeIf { it in MIN_PORT..MAX_PORT }
        )
    }
    ...
    fun isCompleteFor(location: LocationConfig? = null): Boolean {
        ...
        return when (normalized.normalizedType) {
            TYPE_OLCRTC -> location?.normalized()?.isComplete() == true
            TYPE_VLESS -> normalized.localSocksPort != null ||
                normalized.rawConfig?.startsWith("vless://", ignoreCase = true) == true ||
                normalized.uri?.startsWith("vless://", ignoreCase = true) == true
            TYPE_AMNEZIA_WG -> !normalized.rawConfig.isNullOrBlank() ||
    ...
    fun withFallbackName(fallback: String): VpnProfileConfig {
        return if (!name.isNullOrBlank()) this else copy(name = fallback.cleanMetadataValue())
    }

    companion object {
        const val TYPE_OLCRTC = "olcrtc"
        ...
        fun normalizeType(value: String): String {
            return when (value.trim().lowercase()) {
                TYPE_OLCRTC, "olc", "olcbox", "rtc" -> TYPE_OLCRTC
        ...
        fun olcRtc(name: String = ""): VpnProfileConfig {
...
data class LocationEntry(
    ...
    @SerialName("vpn_profile")
    val vpnProfile: VpnProfileConfig? = null,
    ...
    val profile: VpnProfileConfig
        get() = vpnProfile?.normalized() ?: VpnProfileConfig.olcRtc(location.displayName())

    fun displayName(): String = profile.displayName(location.displayName())

    fun isComplete(): Boolean = profile.isCompleteFor(location)

    fun normalized(): LocationEntry {
        val profileConfig = vpnProfile?.normalized()
        if (profileConfig != null && !profileConfig.isOlcRtc()) {
            val normalizedName = profileConfig.displayName(name)
            return LocationEntry(
                storageId = storageId.trim(),
                name = normalizedName,
                subscriptionUrl = firstNotBlank(subscriptionUrl, legacySubscriptionUrl).ifBlank { null },
                vpnProfile = profileConfig.withFallbackName(normalizedName),
                metadata = metadata
                    ?.normalized()
                    ?.takeUnless { it.isEmpty() }
            )
        ...
        return LocationEntry(
            ...
            vpnProfile = profileConfig?.takeIf { !it.isOlcRtc() },
            metadata = metadata
                ?.normalized()
                ?.takeUnless { it.isEmpty() }
        )
    ...
    companion object {
        ...
        fun fromProfile(
            ...
            val displayName = normalized.displayName(storageId)
            return LocationEntry(
                storageId = storageId,
                name = displayName,
                subscriptionUrl = subscriptionUrl,
                vpnProfile = normalized.withFallbackName(displayName),
                metadata = metadata
            ).normalized()
        }
